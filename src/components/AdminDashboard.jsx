...
export default function AdminDashboard({ onLogout }) {
  ...
  async function fetchAdminData() {
    setLoading(true);
    const { data: profiles } = await supabase.from('profiles').select('*, vehicles(id)').order('created_at', { ascending: false });
    if (profiles) {
      setMembers(profiles);
      setStats({ total: profiles.length, pending: profiles.filter(p => !p.is_active_member).length, revenue: profiles.filter(p => p.is_active_member).length * 195 });
    }
    setLoading(false);
  }
  ...
  const handleUpdateRank = async (userId, newRank) => {
    setIsSaving(true);
    const { error } = await supabase.from('profiles').update({ membership_rank: newRank }).eq('id', userId);
    if (!error) { setSelectedUser(null); await fetchAdminData(); }
    setIsSaving(false);
  };
  ...
  return (
    <div className="min-h-screen bg-white pt-28 pb-20 px-8 md:px-20 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <UserEditModal isOpen={!!selectedUser} user={selectedUser} isSaving={isSaving} onClose={() => setSelectedUser(null)} onSave={handleUpdateRank} />
      </div>
    </div>
  );
}
